...
impl Engine {
    ...
    ///
    /// Recognises `postgres://` / `postgresql://`. Engines that are planned but
    /// not yet implemented produce a clear, actionable error rather than a
    ...
    pub fn from_url(url: &str) -> Result<Engine> {
        ...
        match scheme.as_str() {
            ...
            "mysql" | "mariadb" => {
                bail!("MySQL/MariaDB support is planned but not implemented yet (supported: postgres)")
            }
            "sqlite" | "file" => {
                bail!("SQLite support is planned but not implemented yet (supported: postgres)")
            }
            "" => bail!("could not determine the database engine from the connection string"),
            other => {
                bail!("unsupported database engine '{other}://' (supported: postgres)")
            }
        }
    }
}
...
mod tests {
    ...
    fn planned_engines_error_clearly() {
        let err = Engine::from_url("mysql://h/db").unwrap_err().to_string();
        assert!(err.contains("MySQL"), "got: {err}");
        assert!(Engine::from_url("sqlite://./x.db").is_err());
    }
